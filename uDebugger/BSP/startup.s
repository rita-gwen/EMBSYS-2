...
;       (NOTE that IAR needs the name to be  __vector_table for the debugger and C-Spy to work correctly)
      ...
      EXTERN  FaultPrint
      EXTERN  GenerateFault
      EXTERN  PrintExceptionStackFrame
...
//This is as per requirements implementation: extract and print PC and LR
HardFaultIrqHandler1

      ldr       r1, [sp, #20]   //Load LR value from the exception stack frame into R1
      ldr       r0, [sp, #24]   //Load PC value from the exception stack frame into R0
      push      {lr}            //Save
      BL        FaultPrint
      pop       {lr}            //...and restore exception LR
      LDR       R1,=GenerateFault + 2   //return into GenerateFault function
      str       r1, [sp, #24]
      // Return from exception handler
      BX        LR

//This handler prints whole exception stack frame
HardFaultIrqHandler
      mov       r0, sp          //Pass stack top to the C printing function
      mrs       r1, ipsr        //Get the current IPSR value to identify the interrupt type
      push      {lr}            //Save
      BL        PrintExceptionStackFrame
      pop       {lr}            //...and restore exception LR
